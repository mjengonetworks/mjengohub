...
class BaseService extends GetConnect {
  ...
  Future<Response> postRequestNoAuth(
    String path,
    dynamic body, {
    String? contentType,
  }) async {
    ...
    try {
      String cleanPath = path.replaceAll(RegExp(r'^/+'), '');
      print(
        'Making unauthenticated POST request to: ${httpClient.baseUrl}$cleanPath',
      );
      print('Request body type: ${body.runtimeType}');
  ...
  Future<Response> deleteWithBodyRequest(
    String path,
    ...
    try {
      String cleanPath = path.replaceAll(RegExp(r'^/+'), '');
      print(
        'Making DELETE (with body) request to: ${httpClient.baseUrl}$cleanPath',
      );
  ...
  Response _handleResponse(Response response) {
    ...
    if (code >= 400) {
      ...
      return Response(
        ...
        body:
            response.body ??
            {'error': 'CLIENT_ERROR', 'message': 'Bad request.'},
      );
    }
  ...
  Future<bool> testConnection() async {
    try {
      ...
      final baseWithoutPath = httpClient.baseUrl!.replaceAll(
        RegExp(r'api/v1/?$'),
        '',
      );
  ...
  Future<bool> testFlutterEndpoint() async {
    try {
      ...
      final response = await post('test-flutter', {
