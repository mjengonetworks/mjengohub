...
class EcosystemLinksRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if ((mjengoUrl == null || mjengoUrl.isEmpty) &&
        (barabaraUrl == null || barabaraUrl.isEmpty)) {
        ...
        children: [
          if (mjengoUrl != null && mjengoUrl.isNotEmpty)
            _EcosystemChip(
              ...
              onTap: () => LinkLauncher.openLink(context, mjengoUrl),
            ),
          if (barabaraUrl != null && barabaraUrl.isNotEmpty)
            _EcosystemChip(
              label: 'Share Barabara',
              icon: Icons.directions_car_filled_rounded,
              onTap: () => LinkLauncher.openLink(context, barabaraUrl),
            ),
        ],
      ),
    );
...
class MyProjectSubmissionsRow extends StatefulWidget {
  ...
  @override
  State<MyProjectSubmissionsRow> createState() =>
      _MyProjectSubmissionsRowState();
}
...
class MyFollowedProjectsPreview extends StatefulWidget {
  ...
  @override
  State<MyFollowedProjectsPreview> createState() =>
      _MyFollowedProjectsPreviewState();
}

class _MyFollowedProjectsPreviewState extends State<MyFollowedProjectsPreview> {
  final _service = ProjectsService();
  late final Future<List<Project>> _future = _service.getFollowedProjects(
    perPage: 5,
  );
      ...
      builder: (context, snap) {
        ...
        return _PreviewSection(
          ...
          onViewAll: items.isEmpty
              ? null
              : () => Get.to(() => const FollowedProjectsScreen()),
          child: items.isEmpty
    ...
    return FutureBuilder<List<Article>>(
      ...
      builder: (context, snap) {
        ...
        return _PreviewSection(
          ...
          onViewAll: items.isEmpty
              ? null
              : () => Get.to(() => const SubmissionsScreen(initialTab: 0)),
          child: items.isEmpty
    ...
    return FutureBuilder<List<MyComment>>(
      ...
      builder: (context, snap) {
        ...
        return _PreviewSection(
          ...
          onViewAll: items.isEmpty
              ? null
              : () => Get.to(() => const SubmissionsScreen(initialTab: 4)),
          child: items.isEmpty
...
class SiteSafetySubmissionsCard extends StatefulWidget {
  ...
  @override
  State<SiteSafetySubmissionsCard> createState() =>
      _SiteSafetySubmissionsCardState();
}

class _SiteSafetySubmissionsCardState extends State<SiteSafetySubmissionsCard> {
  final _service = IncidentsService();
  late final Future<List<Incident>> _future = _service
      .getMyIncidents(perPage: 50)
      .then((l) => l.where((i) => !i.isRoadSafety).toList());
    ...
    return FutureBuilder<List<Incident>>(
      ...
      builder: (context, snap) {
        ...
        return _PreviewSection(
          ...
          onViewAll: items.isEmpty
              ? null
              : () => Get.to(() => const SubmissionsScreen(initialTab: 3)),
          child: items.isEmpty
              ? _emptyLine(
                  'Site safety incidents you\'ve reported will show up here.',
                )
              : Column(
                  children: items
                      .take(5)
