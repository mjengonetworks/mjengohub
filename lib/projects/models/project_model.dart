...
class Project {
  ...
  /// `updated_at` — unconfirmed against the live backend (no sampled
  /// `GET projects/{slug}` response has carried this key as of the
  /// 2026-09 sessions), parsed defensively like `documents`/`stakeholders`
  /// were before they went live. Null until/unless the API actually sends
  /// it; callers should fall back to [createdAt] rather than fabricate a
  /// date.
  ...
  /// Only [isFollowing] and the vote counts are ever patched client-side
  /// (optimistic follow toggle, optimistic vote) — every other field stays
  /// a direct copy so the currently displayed detail (media, milestones,
  /// team members, ...) never disappears from an unrelated local update.
  Project copyWith({bool? isFollowing, int? upvoteCount, int? downvoteCount}) =>
      Project(
        id: id,
        title: title,
        slug: slug,
