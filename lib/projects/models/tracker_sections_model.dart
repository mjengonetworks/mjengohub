...
class TrackerSections {
  ...
  final List<TrackerSectionGroup>?
  statusPreview; // null for Built History (no status dimension)
  final List<MostViewedWindow> mostViewedWindows;

  const TrackerSections({
    this.categoryPreview = const [],
    this.statusPreview,
    this.mostViewedWindows = const [],
  });

  factory TrackerSections.fromJson(Map<String, dynamic> j) => TrackerSections(
    categoryPreview:
        (j['category_preview'] as List?)
            ?.whereType<Map<String, dynamic>>()
            .map(TrackerSectionGroup.fromJson)
            .toList() ??
        [],
    statusPreview: (j['status_preview'] as List?)
        ?.whereType<Map<String, dynamic>>()
        .map(TrackerSectionGroup.fromJson)
        .toList(),
    mostViewedWindows:
        (j['most_viewed_windows'] as List?)
            ?.whereType<Map<String, dynamic>>()
            .map(MostViewedWindow.fromJson)
            .toList() ??
        [],
  );
